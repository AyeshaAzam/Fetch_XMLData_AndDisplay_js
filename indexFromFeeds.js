const newsItems2 = document.getElementById("newsItems2");

const XMLDataFromFeed = () => {
  ...
  const getAllUrlsData = async () => {
    try {
      ...
      res.map((newItem) => {
        ...
        var items = newsItemList;
        for (var i in items) {
          if (items[i].nodeType == 1) {
            // get rid of the whitespace text nodes
            newsItemsArr.push(items[i]);
          }
        }
      });
      ...
      console.log("unique", uniqueNewsList);

      let tenLatestNews = Array.prototype.slice.call(newsItemsArr).slice(0, 10);
      console.log("tenLatestNews", tenLatestNews);
